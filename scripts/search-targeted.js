...
async function searchAndDownloadSpecific(query, folder) {
...
async function main() {
  await searchAndDownloadSpecific('"slip" OR "culotte" OR "panties" "underwear"', 'panties_exact');
  await searchAndDownloadSpecific('"sewing machine" fashion atelier OR "dressmaker"', 'custom_atelier');
  await searchAndDownloadSpecific('"hangtag" OR "garment label" OR "apparel label"', 'labels_exact');
  await searchAndDownloadSpecific('"bra" "lingerie" "mannequin"', 'bra_mannequin');
}
